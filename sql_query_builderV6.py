...
def build_query(table, conditions, joins=None):
...
def build_query_display(table, conditions, joins=None):
    ...
    with col_sql:
        with st.expander("🧾 Voir la requête SQL générée", expanded=False):
            st.markdown(f"<div class='sql-display'>"
                        f"{build_query_display(current_table, st.session_state.conditions, st.session_state.joins)}"
                        f"</div>", unsafe_allow_html=True)
        st.markdown("<div style='margin-top:16px;'></div>", unsafe_allow_html=True)
        if st.button("▶ Exécuter la requête", width="stretch", type="primary"):
            q, params = build_query(current_table, st.session_state.conditions, st.session_state.joins)
            try:
...
# CONFIGURATION MÉTIER  —  modifier ici pour changer les tables / colonnes
# ══════════════════════════════════════════════════════════════════════════════
# SCHEMA : chaque table déclare ses colonnes, sa clé primaire et ses clés
#          étrangères sortantes. Les jointures sont détectées automatiquement.
SCHEMA = {
    "clients": {
        "columns": ["id", "nom", "prenom", "email", "telephone", "ville", "pays",
                    "date_inscription", "statut"],
        "pk": "id",
        "fk": [],          # pas de FK sortante
    },
    "voyages": {
        "columns": ["id", "client_id", "destination", "pays_destination", "continent",
                    "date_depart", "date_retour", "duree_jours", "type_voyage",
                    "transport", "hotel", "budget", "statut", "note"],
        "pk": "id",
        "fk": [{"col": "client_id", "ref": "clients", "ref_col": "id"}],
    },
}
